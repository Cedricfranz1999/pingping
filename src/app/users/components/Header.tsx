...
import { api } from "~/trpc/react"; // Add TRPC import
...
const Header = () => {
  ...
  // Add cart functionality for users
  const [cartItemCount, setCartItemCount] = useState(0);
  const userId = user?.userId;

  // Get cart data for users
  const { data: cartData } = api.ordersProduct.getCart.useQuery(
    { userId: userId! }, 
    { 
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="hidden sm:block">
          ...
          <p className="text-sm text-gray-600">
            {role === "admin" ? "Admin Dashboard" : role === "user" ? "User Dashboard" : "Employee Portal"}
          </p>
        </div>
      </div>
